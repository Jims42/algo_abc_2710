...
public class Fraction {
	...
	public Fraction difference(Fraction _uneFraction) {
		// int
		// term1=this.numerateur*_uneFraction.getDenominateur()-this.denominateur*_uneFraction.getNumerateur();
		// int term2=this.denominateur*_uneFraction.getDenominateur();
	...
	public Fraction multiFraction(Fraction _autreFraction) {
		int nouvNum = this.numerateur * _autreFraction.getNumerateur();
		int nouvDenom = this.denominateur * _autreFraction.getDenominateur();
		Fraction nouvelFraction = new Fraction(nouvNum, nouvDenom);

		return nouvelFraction;
	}

	public Fraction quotient(Fraction _uneFraction) {
		Fraction nouFraction;
		_uneFraction.inverse();
		nouFraction = this.multiFraction(_uneFraction);
		_uneFraction.inverse();
		return nouFraction;

	}

	public Fraction puissanceFraction(int exposant) {
		int numPuissance = (int) Math.pow(this.numerateur, exposant);
		int denomPuissance = (int) Math.pow(this.denominateur, exposant);
		return new Fraction(numPuissance, denomPuissance);
	}
